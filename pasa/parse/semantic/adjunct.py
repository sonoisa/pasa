...
class Adjunct(object):
    ...
    def _get_adjunct(self, chunk):
        adjunct = ""
        if not adjunct:
            adjunct = self._parse_time(chunk)
        if not adjunct:
            adjunct = self._parse_location(chunk)
        if not adjunct:
            adjunct = self._parse_scene(chunk)
        if not adjunct:
            adjunct = self._parse_instrument(chunk)
        if not adjunct:
            adjunct = self._parse_reason(chunk)
        if not adjunct:
            adjunct = self._parse_limit(chunk)
        if not adjunct:
            adjunct = self._parse_premise(chunk)     # 未定義
        if not adjunct:
            adjunct = self._parse_purpose(chunk)
        if not adjunct:
            adjunct = self._parse_modificand(chunk)  # 未定義
        if not adjunct:
            adjunct = self._parse_manner(chunk)      # 未定義
        return adjunct
